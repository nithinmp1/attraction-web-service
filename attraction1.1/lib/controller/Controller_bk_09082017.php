	...
	function _getConfigData($_AgivenInput)
	{
		...
		$sql = "SELECT
					...
					FROM
					  ...
					  INNER JOIN markup_country_mapping mcm ON mcm.markup_id = md.markup_id
					WHERE
					  md.markup_status = 'Y'
					  AND (md.api = 0 OR md.api = '".$GLOBALS['CONF']['site']['apiId']."')
					  AND (mam.account_id = 0 OR mam.account_id = '".$data[0]['account_id']."')
					  AND (mcm.country_id = 0 OR country_id = 1233333)";

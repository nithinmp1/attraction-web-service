...
class Mail_amazonses extends Mail {
     ...
     */
    function Mail_amazonses($params = null)
    {
        if (isset($params['accessKey'])) $this->accessKey = $params['accessKey'];
        if (isset($params['secretKey'])) $this->secretKey = $params['secretKey'];
    ...
    function send($recipients, $headers, $body)
    {
        ...
        $ses = new SimpleEmailService($this->accessKey, $this->secretKey);
		$ses->enableVerifyHost(false);
		$ses->enableVerifyPeer(false);
		$m = new SimpleEmailServiceMessage();
		if(isset($headers['To'])){
			$emailIds = explode(';',$headers['To']);
			foreach($emailIds as $id)
				$m->addTo($id);
		}
		if(isset($headers['Cc'])){
			$emailIds = explode(';',$headers['Cc']);
			foreach($emailIds as $id)
				$m->addCc($id);
		}
		if(isset($headers['Bcc'])){
			$emailIds = explode(';',$headers['Bcc']);
			foreach($emailIds as $id)
				$m->addBcc($id);
		}
		if(isset($headers['Reply-To']))
			$m->addReplyTo($headers['Reply-To']);
		if(isset($headers['From']))
			$m->setFrom($headers['From']);
		$m->setSubject($headers['Subject']);
		$m->setMessageFromString('',$body);
		
		$ok = $ses->sendEmail($m);
		if(!$ok)
        	return PEAR::raiseError('Amazon SES returned failure');
		else
			return true;
    }
...
class SimpleEmailService
{
	...
	*/
	...
	{
		if($error == false) {
			trigger_error(sprintf("SimpleEmailService::%s(): Encountered an error, but no description given", $functionname), 
E_USER_WARNING);
		}
		...
		{
			$e = $error['Error'];
			$message = sprintf("SimpleEmailService::%s(): %s - %s: %s\nRequest Id: %s\n", $functionname, $e['Type'], $e['Code'], 
$e['Message'], $error['RequestId']);
...
final class SimpleEmailServiceRequest
{
	...
	* PHP's rawurlencode() follows RFC 1738, not RFC 3986 as required by Amazon. The only difference is the tilde (~), so convert it back after 
rawurlencode
